...
- На сервере есть 2GB swap — билд работает на сервере
- **НЕ используй rsync .next/** — локальный билд несовместим с серверным Next.js
- Если билд на сервере зависает >30 секунд — СРАЗУ проси пользователя перезагрузить сервер
- Перед билдом проверяй swap: `swapon --show`

```bash
# 1. Push код на GitHub
git push origin main

# 2. Pull и билд НА СЕРВЕРЕ
ssh root@178.156.249.177 "cd /opt/leaddrive-v2 && git pull origin main && npm run build && pm2 restart leaddrive-v2"
