...
8. Если клиент просит перевести на оператора/менеджера/человека — добавь [ESCALATE] в ответ.
...
async function handleAiAutoReply(
  ...
  try {
    ...
    const shouldCreateTicket = rawReply.includes("[CREATE_TICKET]")
      || /создайте тикет|создать тикет|create a ticket|tiket yarat|tiket aç/i.test(rawReply)
