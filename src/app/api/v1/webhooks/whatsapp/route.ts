...
8. Если клиент просит перевести на оператора/менеджера/человека, или подтверждает перевод (отвечает "Bəli", "Да", "Yes" на твой вопрос о менеджере) — добавь [ESCALATE] в ответ.
9. Если клиент хочет создать тикет — добавь [CREATE_TICKET] в ответ и подтверди создание.
10. НЕ создавай тикет и НЕ эскалируй автоматически — только когда клиент ЯВНО просит менеджера или тикет. Сначала попытайся помочь сам.`
...
async function handleAiAutoReply(
  ...
  try {
    ...
    if (shouldEscalate || shouldCreateTicket) {
      try {
