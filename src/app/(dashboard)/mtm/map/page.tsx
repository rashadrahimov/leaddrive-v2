                ...
                <div className="space-y-1.5 max-h-[150px] overflow-y-auto">
                  {liveFeed.map(evt => (
                    <div key={evt.id} className="text-[10px] flex items-start gap-1.5">
                      <span className={`mt-0.5 w-1.5 h-1.5 rounded-full flex-shrink-0 ${evt.type === "CHECK_IN" ? "bg-green-500" : evt.type === "ALERT" ? "bg-red-500" : "bg-blue-500"}`} />
                      <div>
                        <span className="font-medium">{evt.agent}</span>
                        <span className="text-muted-foreground"> {evt.type === "ALERT" ? "⚠ " : "→ "}{evt.customer}</span>
