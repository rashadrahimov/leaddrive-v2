...
const reportTypeMeta = [
  { id: "daily", icon: Calendar, color: "text-amber-500 bg-amber-50 dark:bg-amber-950/30" },
  { id: "agent", icon: Users, color: "text-blue-500 bg-blue-50 dark:bg-blue-950/30" },
  { id: "route", icon: Route, color: "text-purple-500 bg-purple-50 dark:bg-purple-950/30" },
  { id: "visit", icon: MapPin, color: "text-red-500 bg-red-50 dark:bg-red-950/30" },
  { id: "gps", icon: Satellite, color: "text-cyan-500 bg-cyan-50 dark:bg-cyan-950/30" },
  { id: "photo", icon: Camera, color: "text-green-500 bg-green-50 dark:bg-green-950/30" },
]
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm"><Download className="h-4 w-4 mr-1" /> {t("export")}</Button>
        </div>
      </div>

      {!selectedType ? (
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {reportTypeMeta.map(rt => {
            const Icon = rt.icon
            ...
            return (
              <div key={rt.id} className="rounded-lg border bg-card p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between">
                  ...
                  <span className="text-xs px-2 py-0.5 rounded-full bg-muted text-muted-foreground">{t("records", { count })}</span>
                </div>
                <h3 className="font-semibold mt-3">{t(`type_${rt.id}`)}</h3>
                <p className="text-sm text-muted-foreground mt-1">{t(`type_${rt.id}_desc`)}</p>
                <div className="flex items-center justify-between mt-4">
                  <span className="text-[10px] text-muted-foreground">{t("lastGenerated", { date: new Date().toLocaleDateString() })}</span>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => setSelectedType("")}>{t("backToReports")}</Button>
            <h3 className="font-semibold">{t(`type_${selectedType}`)}</h3>
          </div>

          {reportData.length === 0 ? (
            <div className="h-48 flex items-center justify-center text-muted-foreground border rounded-lg bg-card">{t("noData")}</div>
          ) : (
                  ...
                  <tr className="border-b bg-muted/50 text-left">
                    {selectedType === "agent" ? (
                      <>
                        ...
                        <th className="px-4 py-2 font-medium">{t("colVisits")}</th>
                        <th className="px-4 py-2 font-medium">{t("colTasks")}</th>
                        <th className="px-4 py-2 font-medium">{t("colPhotos")}</th>
                      </>
                    ) : selectedType === "visit" ? (
                      <>
                        <th className="px-4 py-2 font-medium">{t("colAgent")}</th>
