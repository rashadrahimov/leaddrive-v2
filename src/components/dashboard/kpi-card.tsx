...
export function KpiCard({ title, value, sub, icon, color, alert }: {
      ...
      <div className="relative p-3 pt-4">
        <div className="flex justify-between items-start">
          <div className="min-w-0">
            ...
            <p className="text-xl font-bold mt-1 tabular-nums leading-none"><AnimatedNumber value={value} /></p>
            {sub && <p className="text-[11px] text-muted-foreground mt-1 leading-tight">{sub}</p>}
          </div>
          <div className="p-2 rounded-xl shrink-0 ml-2" style={{ color, backgroundColor: `${color}12` }}>{icon}</div>
        </div>
      </div>
    </div>
  )
