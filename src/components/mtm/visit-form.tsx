...
export function MtmVisitForm({ open, onOpenChange, onSaved, initialData, orgId }: VisitFormProps) {
  ...
  useEffect(() => {
    if (open) {
      setForm({
        ...
        status: initialData?.status || "CHECKED_IN",
        notes: initialData?.notes || "",
        latitude: initialData?.checkInLat?.toString() || "",
        longitude: initialData?.checkInLng?.toString() || "",
      })
      setError("")
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogHeader>
        <DialogTitle>{isEdit ? "Edit Visit" : "Log Visit"}</DialogTitle>
      </DialogHeader>
