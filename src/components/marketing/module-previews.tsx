...
function Sidebar({ items, activeLabel }: { items?: SidebarItem[]; activeLabel?: string }) {
  ...
  return (
    <div className="w-[100px] flex-shrink-0 bg-slate-900 border-r border-slate-700/50 py-2 px-1 hidden sm:block">
      <div className="flex items-center gap-1 px-1.5 mb-2">
        <div className="w-3 h-3 bg-gradient-to-br from-orange-400 to-red-500 rounded-md" />
        <span className="font-bold text-[6px] text-white/90">LeadDrive</span>
      </div>
      <div className="relative mb-2 px-0.5">
        <Search className="absolute left-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 text-slate-500" />
        <div className="bg-slate-800 rounded-md py-0.5 pl-4 text-slate-500 text-[5px]">Axtar...</div>
      </div>
      {list.map((item, i) => {
        ...
        return (
          <div key={i}>
            {item.section && (
              <div className="text-[4px] font-semibold text-slate-500 uppercase tracking-wider px-1.5 mt-1.5 mb-0.5">{item.section}</div>
            )}
            <div className={cn(
...
function TopBar() {
  return (
    <div className="flex items-center justify-between px-2.5 py-1 bg-white border-b border-slate-200">
      <span className="text-[7px] font-semibold text-slate-400">Güvən Technology LLC</span>
      <div className="flex items-center gap-1">
        <Bell className="w-2 h-2 text-slate-400" />
        <div className="w-3.5 h-3.5 bg-gradient-to-br from-orange-400 to-red-500 rounded-full flex items-center justify-center text-[5px] font-bold text-white">R</div>
      </div>
    </div>
  )
}
...
const pipelineColumns = [
  ...
  {
    title: "Danışıq", color: "bg-orange-500", count: 2, total: "₼27,600",
    deals: [
      ...
      <div className="flex-1 bg-slate-50 text-slate-900 overflow-hidden">
        ...
        <div className="p-2">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <div className="flex items-center gap-1">
              <span className="text-[5px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-200">Ön yarat</span>
              <span className="bg-orange-500 rounded px-1.5 py-0.5 text-[5px] text-white font-medium">+ Yeni sövdələşmə</span>
            </div>
          </div>
            ...
            {pipelineColumns.map((col) => (
              <div key={col.title} className="flex-1 min-w-0">
                <div className="flex items-center gap-0.5 mb-1">
                  ...
                  <span className="text-[6px] font-semibold text-slate-700">{col.title}</span>
                  <span className="text-[5px] text-slate-400 ml-auto">{col.count}</span>
                </div>
                <div className="space-y-0.5">
                  {col.deals.map((deal, j) => (
                    <div key={j} className="bg-white rounded border border-slate-200 p-1 shadow-sm">
                      <div className="text-[6px] font-semibold text-slate-800 truncate">{deal.name}</div>
                      <div className="text-[5px] text-slate-400 truncate">{deal.company}</div>
                      <div className="flex items-center justify-between mt-0.5">
                        <span className="text-[6px] font-bold text-emerald-600">{deal.amount}</span>
                        <span className="text-[5px] text-slate-400">{deal.prob}%</span>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <div className="flex-1 bg-slate-50 text-slate-900 overflow-hidden">
        ...
        <div className="p-2">
          ...
          <div className="flex items-center gap-1 mb-1">
            ...
            <div>
              <div className="flex items-center gap-1">
                <span className="text-[9px] font-bold text-slate-900">ERP Tətbiqi</span>
                <span className="bg-cyan-100 text-cyan-700 text-[5px] font-medium px-1 py-[1px] rounded">Təklif</span>
              </div>
              <span className="text-[5px] text-slate-500">NeftGaz MMC</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-1 mb-1.5">
            <div className="bg-white rounded-lg border border-slate-200 p-1">
              <div className="flex justify-between mb-0.5">
                <span className="text-[5px] font-medium text-slate-700">Ehtimal</span>
                <span className="text-[5px] font-semibold text-slate-900">60%</span>
              </div>
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-1">
              <div className="flex justify-between mb-0.5">
                <span className="text-[5px] font-medium text-slate-700">Güvən səviyyəsi</span>
                <span className="text-[5px] font-semibold text-slate-900">75%</span>
              </div>
              <div className="h-1 bg-slate-100 rounded-full overflow-hidden">
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-1.5">
            <div className="flex items-center gap-1 mb-1">
              ...
              <span className="text-[6px] font-bold text-slate-900">Next Best Offers</span>
            </div>
            <div className="space-y-1">
              {dealOffers.map((o, i) => (
                <div key={i} className="flex items-center gap-1.5 p-0.5 rounded bg-slate-50 border border-slate-100">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[5px] font-semibold text-slate-900">{o.name}</div>
                    <div className="text-[4px] text-slate-500 truncate">{o.desc}</div>
                  </div>
                  <span className="text-[5px] font-medium text-slate-700">{o.price}</span>
                  <div className={cn(
                    "w-5 h-5 rounded-full flex items-center justify-center text-[4px] font-bold flex-shrink-0",
                    o.match >= 90 ? "bg-emerald-100 text-emerald-700" : o.match >= 80 ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-600"
                  )}>{o.match}%</div>
                </div>
              ))}
            </div>
...
const campaigns = [
  ...
  { name: "Pharmastore News", type: "Email", status: "Qaralama", statusColor: "bg-slate-100 text-slate-700", sent: 0, opened: 0, clicked: 0, converted: 0 },
    ...
    <div className="flex text-[7px] leading-tight bg-slate-900 text-white min-h-[280px] select-none">
      ...
      <div className="flex-1 bg-slate-50 text-slate-900 overflow-hidden">
        ...
        <div className="p-2">
          <div className="flex items-center justify-between mb-1.5">
            <div>
              <h2 className="text-[9px] font-bold text-slate-900">Kampaniyalar</h2>
              <p className="text-[5px] text-slate-500">E-poçt/SMS kampaniyaları yaradın və göndərin</p>
            </div>
            <span className="bg-orange-500 rounded px-1.5 py-0.5 text-[5px] text-white font-medium">+ Yeni kampaniya</span>
          </div>
          ...
          <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
            <div className="grid grid-cols-[1.5fr_0.5fr_0.6fr_0.5fr_0.5fr_0.5fr_0.5fr] gap-0.5 px-1.5 py-1 bg-slate-50 border-b border-slate-200 text-[5px] font-semibold text-slate-500 uppercase">
              <span>Kampaniya</span>
              <span>Tip</span>
              <span>Status</span>
            ...
            {campaigns.map((c, i) => (
              <div key={i} className={cn(
                ...
                i < campaigns.length - 1 && "border-b border-slate-100"
              )}>
                ...
                <span className="text-center text-slate-700">{c.sent > 0 ? c.sent.toLocaleString() : "—"}</span>
                <span className="text-center text-slate-700">{c.opened > 0 ? c.opened.toLocaleString() : "—"}</span>
                <span className="text-center text-slate-700">{c.clicked > 0 ? c.clicked.toLocaleString() : "—"}</span>
...
const scoredLeads = [
  ...
  { name: "Aynur Quliyeva", company: "FinServ Group", score: 34, grade: "D", gradeColor: "bg-orange-500", source: "kampaniya", conv: "22%" },
    ...
    <div className="flex text-[7px] leading-tight bg-slate-900 text-white min-h-[280px] select-none">
      ...
      <div className="flex-1 bg-slate-50 text-slate-900 overflow-hidden">
        ...
        <div className="p-2">
          ...
          <div className="text-[5px] text-slate-500 mb-1">Ort. bal: 66/100 &nbsp; Ehtimal: 61% &nbsp; Cəmi sessiyalar: 8</div>

          {/* Lead scoring table */}
          <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
            <div className="flex items-center justify-between px-1.5 py-1 bg-slate-50 border-b border-slate-200">
              <span className="text-[6px] font-bold text-slate-700">Lidlər</span>
            </div>
            <div className="grid grid-cols-[0.3fr_1.2fr_0.8fr_0.5fr_0.6fr] gap-0.5 px-1.5 py-0.5 border-b border-slate-100 text-[5px] font-semibold text-slate-400 uppercase">
              <span>Bal</span>
              <span>Lid</span>
              <span>Şirkət</span>
              <span>Mənbə</span>
            ...
            {scoredLeads.map((l, i) => (
              <div key={i} className={cn(
                ...
                i < scoredLeads.length - 1 && "border-b border-slate-50"
              )}>
                <div className="flex items-center gap-0.5">
                  ...
                  <span className="text-[5px] text-slate-500">{l.score}</span>
                </div>
                <span className="text-slate-800 font-medium truncate">{l.name}</span>
                ...
                <div className="flex items-center gap-0.5">
                  ...
                  <span className="text-[5px] text-slate-500">{l.conv}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="p-2">
          ...
          <div className="flex gap-[2px] mb-1.5">
            {["Yeni", "Açıq", "Gözləyir", "Güncəlləndi", "Həll olundu", "Bağlı"].map((s, i) => (
              <div key={i} className={cn(
                "flex-1 py-[2px] text-center text-[5px] font-medium rounded-sm",
                s === "Açıq" ? "bg-blue-500 text-white" : "bg-slate-200 text-slate-500"
              )}>{s}</div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-[1.5fr_1fr] gap-1.5">
            {/* Conversation */}
            <div className="bg-white rounded-lg border border-slate-200 p-1.5 space-y-1">
              ...
              <div className="flex gap-0.5 mt-0.5">
                <span className="bg-slate-100 rounded px-1 py-[1px] text-[5px] text-slate-600">Geri</span>
            ...
            <div className="space-y-1">
              <div className="bg-white rounded-lg border border-slate-200 p-1.5">
                <div className="text-[6px] font-bold text-slate-700 mb-1">Detallar</div>
                {[
                  { l: "Statusu", v: "Açıq", c: "text-blue-600" },
                  { l: "Prioritet", v: "Yüksək", c: "text-orange-600" },
                  { l: "Kateqoriya", v: "ai_escalation", c: "text-slate-600" },
                  { l: "Yaradılma", v: "21.03.2026", c: "text-slate-500" },
                ].map((d) => (
                  <div key={d.l} className="flex justify-between py-[1px]">
                    <span className="text-[5px] text-slate-400">{d.l}</span>
              ...
              <div className="bg-white rounded-lg border border-slate-200 p-1.5">
                <div className="flex items-center gap-0.5 mb-0.5">
                  ...
                  <span className="text-[6px] font-bold text-slate-700">SLA</span>
                </div>
                <div className="flex justify-between py-[1px]">
                  <span className="text-[5px] text-slate-400">Son tarix</span>
...
const profitClients = [
  ...
  { name: "Digər", value: 28000, color: "bg-slate-400" },
]
    ...
    <div className="flex text-[7px] leading-tight bg-slate-900 text-white min-h-[280px] select-none">
      ...
      <div className="flex-1 bg-slate-50 text-slate-900 overflow-hidden">
        ...
        <div className="p-2">
          <div className="flex items-center justify-between mb-1.5">
            <div>
              <h2 className="text-[9px] font-bold text-slate-900">Gəlirlilik</h2>
              <p className="text-[5px] text-slate-500">Xərc modeli analitikası: gəlir, xərclər, mənfəət</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-1.5">
              <div className="text-[6px] font-bold text-slate-700 mb-1">Xərc strukturu</div>
              ...
              <div className="flex h-2 rounded-full overflow-hidden mb-1.5">
                ...
                <div className="bg-slate-400" style={{ width: "5%" }} />
              </div>
              <div className="space-y-[2px]">
                {[
                  ...
                  { l: "Digər", p: "5%", c: "bg-slate-400" },
                ].map((s) => (
                  <div key={s.l} className="flex items-center gap-1">
                    <div className={cn("w-1.5 h-1.5 rounded-full", s.c)} />
                    <span className="text-[5px] text-slate-600 flex-1">{s.l}</span>
                    <span className="text-[5px] text-slate-400">{s.p}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-1.5">
              <div className="text-[6px] font-bold text-slate-700 mb-1">Xidmət səviyyəsi üzrə Gəlir</div>
              <div className="space-y-[4px]">
                {profitClients.map((c) => (
                  <div key={c.name}>
                    <div className="flex items-center justify-between mb-[1px]">
                      <span className="text-[5px] text-slate-600 truncate">{c.name}</span>
                      <span className="text-[5px] text-slate-500">₼{(c.value / 1000).toFixed(0)}K</span>
                    </div>
          ...
          <div className="grid grid-cols-3 gap-1 mt-1.5">
            {[
              ...
              <div key={m.l} className="bg-white rounded border border-slate-200 p-1 text-center">
                <div className="text-[5px] text-slate-500">{m.l}</div>
                <div className="text-[7px] font-bold text-slate-800">{m.v}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="flex text-[7px] leading-tight bg-slate-900 text-white min-h-[280px] select-none">
      ...
      <div className="flex-1 bg-slate-50 text-slate-900 overflow-hidden">
        ...
        <div className="p-2">
          <div className="flex items-center justify-between mb-1.5">
            <div>
              <h2 className="text-[9px] font-bold text-slate-900">Büdcələşdirmə</h2>
              <p className="text-[5px] text-slate-500">Büdcə planlaşdırma, proqnoz və icra</p>
            </div>
            <div className="flex items-center gap-1">
              <span className="text-[5px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-200">Q1 2026</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-[1.5fr_1fr] gap-1.5">
            {/* Plan vs Actual bars */}
            <div className="bg-white rounded-lg border border-slate-200 p-1.5">
              <div className="text-[6px] font-bold text-slate-700 mb-1.5">Plan / Fakt kateqoriya üzrə</div>
              <div className="space-y-[6px]">
                {budgetRows.map((r) => (
                  <div key={r.label}>
                    <div className="flex items-center justify-between mb-[1px]">
                      <span className="text-[5px] text-slate-600">{r.label}</span>
              ...
              <div className="flex gap-2 mt-1 text-[4px] text-slate-400">
            ...
            {/* Execution donut */}
            <div className="bg-white rounded-lg border border-slate-200 p-1.5 flex flex-col items-center justify-center">
              <div className="text-[6px] font-bold text-slate-700 mb-1.5">Büdcə icrasI</div>
              ...
              <div className="text-[5px] text-slate-400 mt-1">Büdcə icrası faizi</div>
              <div className="grid grid-cols-2 gap-1 mt-1.5 w-full">
                <div className="text-center p-0.5 bg-slate-50 rounded border border-slate-100">
                  <div className="text-[7px] font-bold text-blue-600">₼2.0M</div>
                  <div className="text-[4px] text-slate-400">Plan</div>
                </div>
                <div className="text-center p-0.5 bg-slate-50 rounded border border-slate-100">
                  <div className="text-[7px] font-bold text-emerald-600">₼1.8M</div>
                  <div className="text-[4px] text-slate-400">Fakt</div>
                </div>
              </div>
            </div>
          </div>
    ...
    <div className="flex text-[7px] leading-tight bg-slate-900 text-white min-h-[280px] select-none">
      ...
      <div className="flex-1 bg-slate-50 text-slate-900 overflow-hidden">
        ...
        <div className="p-2">
          <div className="flex items-center justify-between mb-1.5">
            <div>
              <h2 className="text-[9px] font-bold text-slate-900">Layihələr</h2>
              <p className="text-[5px] text-slate-500">Layihə tapşırıqlarını izləyin, vəzifələri və resursları idarə edin</p>
            </div>
            <span className="bg-orange-500 rounded px-1.5 py-0.5 text-[5px] text-white font-medium">+ Yeni layihə</span>
          </div>
          ...
          {/* Projects table */}
          <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
            <div className="grid grid-cols-[1.5fr_0.7fr_1fr_0.5fr_0.5fr] gap-0.5 px-1.5 py-1 bg-slate-50 border-b border-slate-200 text-[5px] font-semibold text-slate-500 uppercase">
              <span>Layihə</span>
              <span>Status</span>
              <span>Proqres</span>
              <span>Komanda</span>
            ...
            {projects.map((p, i) => (
              <div key={i} className={cn(
                ...
                i < projects.length - 1 && "border-b border-slate-100"
              )}>
                <div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[5px] text-slate-500 w-5 text-right">{p.progress}%</span>
                </div>
                <div className="flex -space-x-1">
                  ...
                  {p.members > 3 && <span className="text-[4px] text-slate-400 ml-1">+{p.members - 3}</span>}
                </div>
                <span className="text-[5px] text-slate-500">{p.deadline}</span>
              </div>
            ))}
          </div>
        </div>
...
const tasks = [
  ...
  { title: "Обновка backlog предложений", priority: "Aşağı", prColor: "bg-slate-400", assignee: "Nigar", status: "Bitmiş", stColor: "bg-emerald-100 text-emerald-700", date: "15 mar" },
    ...
    <div className="flex text-[7px] leading-tight bg-slate-900 text-white min-h-[280px] select-none">
      ...
      <div className="flex-1 bg-slate-50 text-slate-900 overflow-hidden">
        ...
        <div className="p-2">
          <div className="flex items-center justify-between mb-1.5">
            <div>
              <h2 className="text-[9px] font-bold text-slate-900">Tapşırıqlar</h2>
              <p className="text-[5px] text-slate-500">Komanda tapşırıqlarını idarə et</p>
            </div>
            <div className="flex gap-1">
              <span className="text-[5px] px-1 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-200">Kanban</span>
              <span className="text-[5px] px-1 py-0.5 rounded bg-blue-500 text-white">Siyahı</span>
              <span className="bg-orange-500 rounded px-1.5 py-0.5 text-[5px] text-white font-medium">+ Tapşırıq</span>
            </div>
          </div>
          ...
          {/* Tasks table */}
          <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
            <div className="grid grid-cols-[1.5fr_0.5fr_0.5fr_0.6fr_0.4fr] gap-0.5 px-1.5 py-1 bg-slate-50 border-b border-slate-200 text-[5px] font-semibold text-slate-500 uppercase">
              <span>Tapşırıq</span>
              <span>Prioritet</span>
              <span>Təyin ed.</span>
              <span>Status</span>
            ...
            {tasks.map((t, i) => (
              <div key={i} className={cn(
                ...
                <div className="flex items-center gap-0.5">
                  <CheckCircle2 className={cn("w-2 h-2 flex-shrink-0", t.status === "Bitmiş" ? "text-emerald-500" : "text-slate-300")} />
                  <span className="text-slate-800 truncate">{t.title}</span>
                </div>
                <div className="flex items-center gap-0.5">
                  <div className={cn("w-1.5 h-1.5 rounded-full", t.prColor)} />
                  <span className="text-[5px] text-slate-600">{t.priority}</span>
                </div>
                ...
                <span className="text-[5px] text-slate-400">{t.date}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 bg-slate-50 text-slate-900 overflow-hidden">
        ...
        <div className="p-2">
          <div className="mb-1.5">
            <h2 className="text-[9px] font-bold text-slate-900">Sabahınız xeyir, Emil Rahimsoy</h2>
            <p className="text-[5px] text-slate-500">Platformanı konfiqurasiya edin</p>
          </div>
          ...
          <div className="grid grid-cols-4 gap-1 mb-2">
            {[
              ...
              { label: "Yeni Lidlər", value: "11", color: "bg-orange-500" },
            ].map((k, i) => (
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {/* Revenue bar chart */}
            <div className="bg-white rounded-lg border border-slate-200 p-1.5">
              <div className="text-[6px] font-bold text-slate-700 mb-1">Xidmətlər üzrə gəlir</div>
              {[
                ...
                <div key={s.name} className="flex items-center gap-1 mb-[3px]">
                  <span className="text-[5px] text-slate-500 w-12 text-right truncate">{s.name}</span>
                  <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
            ...
            {/* Settings sections */}
            <div className="bg-white rounded-lg border border-slate-200 p-1.5">
              <div className="text-[6px] font-bold text-slate-700 mb-1">Satış pipeline</div>
              <div className="space-y-[3px]">
                {[
                  ...
                  <div key={s.label} className="flex items-center gap-1 p-0.5 rounded hover:bg-slate-50">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-[5px] font-semibold text-slate-800">{s.label}</div>
                      <div className="text-[4px] text-slate-400">{s.desc}</div>
                    </div>
                    <span className="text-[5px] text-slate-400">{s.count}</span>
    ...
    <div className="flex text-[7px] leading-tight bg-slate-900 text-white min-h-[280px] select-none">
      ...
      <div className="flex-1 bg-slate-50 text-slate-900 overflow-hidden">
        ...
        <div className="p-2">
          <div className="flex items-center justify-between mb-1.5">
            <div>
              <h2 className="text-[9px] font-bold text-slate-900">Şirkətlər (59)</h2>
              <p className="text-[5px] text-slate-500">Müştəri şirkətlərini idarə edin</p>
            </div>
            <span className="bg-orange-500 rounded px-1.5 py-0.5 text-[5px] text-white font-medium">+ Əlavə et</span>
          </div>
          ...
          <div className="flex gap-0.5 mb-1.5">
            {["Hamısı (59)", "Aktiv (59)", "Perspektiv (5)", "Qeyri-aktiv (5)"].map((t, i) => (
              <span key={i} className={cn("text-[5px] px-1 py-0.5 rounded", i === 0 ? "bg-blue-500 text-white" : "bg-slate-100 text-slate-500")}>{t}</span>
            ))}
          </div>
          ...
          <div className="grid grid-cols-3 gap-1">
            {companies.map((c) => (
              <div key={c.name} className="bg-white rounded-lg border border-slate-200 p-1.5 shadow-sm">
                <div className="flex items-center justify-between mb-0.5">
                  <span className="text-[6px] font-bold text-slate-800 truncate">{c.name}</span>
                  <span className="text-[5px] text-slate-400">Aktiv</span>
                </div>
                <div className="text-[5px] text-slate-500 mb-1">{c.sector}</div>
                <div className="grid grid-cols-2 gap-0.5 text-[5px]">
                  <div className="flex items-center gap-0.5">
                    <Users className="w-1.5 h-1.5 text-slate-400" />
                    <span className="text-slate-600">{c.contacts} kontakt</span>
                  </div>
                  <div className="flex items-center gap-0.5">
                    <Briefcase className="w-1.5 h-1.5 text-slate-400" />
                    <span className="text-slate-600">{c.deals} sövdələşmə</span>
                  </div>
                </div>
      ...
      <div className="flex-1 bg-slate-50 text-slate-900 overflow-hidden">
        ...
        <div className="flex h-[250px]">
          ...
          <div className="flex-1 p-2 overflow-hidden">
            <div className="mb-1.5">
              <h2 className="text-[9px] font-bold text-slate-900">Sabahınız xeyir, Emil Rahimsoy</h2>
              <p className="text-[5px] text-slate-500">Rəhbər paneli - 2026 M03.28, Şb</p>
            </div>
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-1.5 mb-1">
              <div className="text-[6px] font-bold text-slate-700 mb-1">Xidmətlər üzrə gəlir</div>
              {[
                ...
                <div key={s.name} className="flex items-center gap-1 mb-[2px]">
                  <span className="text-[4px] text-slate-500 w-10 text-right">{s.name}</span>
                  <div className="flex-1 h-1 bg-slate-100 rounded-full overflow-hidden">
      ...
      <div className="flex-1 bg-slate-50 text-slate-900 overflow-hidden">
        ...
        <div className="p-2">
          ...
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-1">
                <span className="text-[9px] font-bold text-slate-900">Tarlan M. Məmmədli (AZM)</span>
              ...
              <span className="text-[5px] text-slate-500">Azmade &bull; tarlan.mammadli@azmade.az</span>
            </div>
          ...
          {/* Lead details */}
          <div className="bg-white rounded-lg border border-slate-200 p-1.5">
            <div className="text-[6px] font-bold text-slate-700 mb-1">Lid məlumatları</div>
            <div className="grid grid-cols-2 gap-x-3 gap-y-[3px]">
              {[
                ...
                <div key={d.l} className="flex items-center gap-1">
                  <span className="text-[5px] text-slate-400 w-14">{d.l}:</span>
                  <span className="text-[5px] text-slate-700 font-medium truncate">{d.v}</span>
                </div>
              ))}
            </div>
          </div>
